...
var last;
var urlWoProto = function( url ){
  var postprotoI = url.indexOf('://');

  return url.substring( postprotoI + 3 );
};
...
tabs.on('pageshow', function( tab ){
  ...
  if( newTab && last ){
    for( var tabId in tabhist ){
      var tabHistToChk = tabhist[ tabId ];
      var prevEntToChk = tabHistToChk[ tabHistToChk.length - 1 ];

      if( prevEntToChk ){
        log('Checking tab ', tabId, prevEntToChk.url, prevEntToChk.linkout);

        if( prevEntToChk.linkout && urlWoProto(prevEntToChk.linkout.url) === urlWoProto(ent.url) ){
          log('Opened new tab from prior tab');

          ent.fromTab = prevEntToChk.id;
        }
      }
    }
  }
